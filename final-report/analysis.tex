...
Developers can choose to run either a context-insensitive
or a context-sensitive version of our analysis.
The context-insensitive analysis always runs first and
produces results that are reused by the context-sensitive
analysis. This makes the context-sensitive analysis
faster, because it reuses summary information produced
by the context-insensitive analysis.
...
is a mapping from each program variable $v$ to all
statements where $v$ might be tainted. If $v$ might not
be tainted at before statment $s$, then $\myinflow_s[v] = \emptyset$.
Otherwise, $\myinflow_s[v]$ contains all statements where $v$
might get tainted.

\paragraph{Context-Insensitive Analysis}
Our context-insensitive version of the analysis is performed
...
\IncMargin{1em}
\begin{algorithm*}
\caption{Context-Insentive Flow Function}\label{fig:algoFlow}
%\SetAlgoNoLine
\SetAlgoLined
\LinesNumbered
\DontPrintSemicolon
...
\Switch{$\Type(\stmt)$}{
	...
	\Case{$\LSOURCE\ [\mathit{call}\ \Func(a_0, a_1, ..., a_n)]$}{
...
\IncMargin{1em}
\begin{algorithm*}
\caption{Context-Sentive Flow Function}\label{fig:contextFlow}
%\SetAlgoNoLine
\SetAlgoLined
\LinesNumbered
\DontPrintSemicolon
...
Context-sensitive		
	\Case{$\LCALL\ [\mathit{call}\ \Func(a_0, a_1, ..., a_n)]$}{
		\If{$\Caller \neq \Func$}{
			\ForEach{$\stmtA_k, k \in \set{0, 1, ..., n}$}{
				$\stmtF_k \leftarrow \Formal(\Func, \stmtA_k)$\;			
