...
\Switch{$\Type(\stmt)$}{
	...
	\Case{$\LCALL\ [call\ \Func(a_0, a_1, ..., a_n)]$}{
		\If{$\Caller \neq \Func$}{
			$\fZero \leftarrow \First(\Func)$\;
			\ForEach{$\stmtA_k, k \in \set{0, 1, ..., n}$}{
				\ForEach{$\stmtA_k, k \in \set{0, 1, ..., n}$}{
				\If{$\stmtA_k \in \mathcal{A}$}{			
					$\Input[\stmtA_k] \leftarrow \Input[\stmtA_k] \cup \set{\stmt}$\;
				}
				\ElseIf{$\stmtA_k \in \mathcal{P}$}{
