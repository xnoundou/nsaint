...
\InData{$\Func: \mathit{Proc}, \InitDataFlow: \mathit{Inst} \rightarrow \mathit{Var}$}
\OutData{$\Void$}
...
\InData{$\Caller: \mathit{Proc}, \stmt: \mathit{Inst}, \Input: \mathit{Var}$}
\OutData{$\Output: \mathit{Var}$}
$\Output \leftarrow \Input$\;
\Switch{$\Type(\stmt)$}{
	\Case{$\LCOPY\ [p = q] $}{
		$\Output \leftarrow \Input \cup \set{p}$\;
	}
	\Case{$\LLOAD\ [p = *q] $}{
		\ForEach{$\stmtV \in \pointsto{\stmt}{q}$}{	
			$\Output \leftarrow \Input \cup \set{\stmtV}$\;
		}
	}
...
%	}
	...
	\Case{$\LSOURCE\ [\Func(a_0, a_1, ..., a_n)]$}{
		\ForEach{$i \in \set{0,1,...,n}$}{
			\If{$i \in \Taint(func)$}{			
				$\Output \leftarrow \Input \cup \set{a_i}$\;
			}			
		}
	}	
