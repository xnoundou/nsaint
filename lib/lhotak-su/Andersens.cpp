         ...
         E = scc_end(rootNode); SCCI != E; ++SCCI) {
           ...
           if (nextSCC.size() > 1 || (nextSCC.size() == 1 && SCCI.hasLoop())) {
             for (std::vector<CallGraphNode*>::const_iterator I = nextSCC.begin(),
               ...
               DEBUG(cerr << "Recursive Function: " << f->getName().str() << std::endl);
             }
           }
      }
    }
...
#undef DEBUG_TYPE
#define DEBUG_TYPE "anders-aa"
      cerr << "Number of labels: " << programPoint << std::endl;
      SolveConstraints();
...
void Andersens::CollectConstraints(Module &M) {
  ...
  const TargetData& targetData = *getTargetData();
  ...
  for (Module::global_iterator I = M.global_begin(), E = M.global_end();
    ...
    if (!t->isSized()) {
      DEBUG(cerr << "GlobalVariable " << I->getName().str() << ", ??? bits" << std::endl);
      Object->Singleton = false;
    } else if (targetData.getTypeSizeInBits(t) > (int)targetData.getPointerSizeInBits()) {
      DEBUG(cerr << "GlobalVariable " << I->getName().str() << ", " << targetData.getTypeSizeInBits(t) << " bits" << std::endl);
      Object->Singleton = false;
    }
...
void Andersens::visitAllocationInst(AllocationInst &AI) {
  ...
  || recursiveFunctions.find(f) != recursiveFunctions.end()) {
    ...
    if(!AI.getAllocatedType()->isSized())
      DEBUG(cerr << "LocalVariable " << AI.getName().str() << ", ??? bits, declared in function " << f->getName().str() << std::endl);
    else
      DEBUG(cerr << "LocalVariable " << AI.getName().str() << ", " << targetData.getTypeStoreSizeInBits(AI.getAllocatedType()) << " bits, declared in function " << f->getName().str() << std::endl);
...
#define DEBUG_TYPE "anders-aa-renumber"
  ...
  cerr << "Number of address taken nodes: " << NewGraphNodes.size() << std::endl;
  ...
  cerr << "Total number of nodes: " << NewGraphNodes.size() << std::endl;
...
#define DEBUG_TYPE "anders-aa"
  ...
  while( !CurrWL->empty() || (SU_ENABLED && !suWL.empty()) ) {
    ...
    cerr << "Starting iteration #" << ++NumIters << "\n";
    cerr << "CurrWL size " << CurrWL->size() << std::endl;
    cerr << "suWL size " << suWL.size() << std::endl;
...
void Andersens::PrintSUSet(unsigned pp) const {
  ...
  cerr << pp << type << ": " << SUPTToString(pp) << std::endl;
  cerr << "pred";
...
void Andersens::PrintSUSets() const {
  ...
  cerr << "Total SU set size: " << size << std::endl;
}
...
void Andersens::Statistics() {
  ...
  cerr << "Stores: " << nulls << " nulls, " << strongs << " strongs, " << stores << " total." << std::endl;
  ...
  cerr << "Loads: " << bottoms << " bottoms, " << empties << " empties, " <<
...
void Andersens::DumpStreams() {
  ...
  dir += "/";
  Streams.open(dir);
  cerr << "printing constraints" << std::endl;
  for (unsigned i = 0, e = Constraints.size(); i != e; ++i) {
